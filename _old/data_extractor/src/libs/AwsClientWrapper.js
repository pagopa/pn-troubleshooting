...
x
  ...
  async _getLogsByTraceIdsMultipleAccount(logGroupNames, fromEpochMs, toEpochMs, traceIds) {
    ...
    if (profile == "core"){
      ...
      query["core"] = await this._scheduleQueryAccount("core", this._ecsLogGroupsNames["core"].concat("/aws/events/core-event-bus-input-events"), queryString, fromEpochMs, toEpochMs)
    }
  ...
  async getEventsByDLQ(queueName, limit) {
