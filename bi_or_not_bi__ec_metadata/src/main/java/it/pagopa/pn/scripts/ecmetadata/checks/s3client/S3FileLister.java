package it.pagopa.pn.scripts.ecmetadata.checks.s3client;
...
public class S3FileLister extends MsgSenderSupport {
    ...
    public Stream<String> getObjectContent( String bucket, S3Object s3Obj ) {
        ...
        try{
            fireMessage(Msg.readFileStart( s3Obj.key() ));

            InputStream is = new GZIPInputStream( new BufferedInputStream( presignedRequest.url().openStream() ));
