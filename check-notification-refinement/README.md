...
2. Per ogni IUN:
   ...
   * Analizza gli eventi restituiti e cerca la presenza di eventi con `category` uguale a `NOTIFICATION_VIEWED`, `NOTIFICATION_CANCELLED` o `REFINEMENT`.
     * Se nessuno dei tre è presente, la notifica è classificata come **Non perfezionata**.
     * Se è presente solo uno dei tre, la notifica è classificata come **Perfezionata**.
     * Se sono presenti più di uno, viene considerato solo quello con timestamp più vecchio e la notifica è classificata come **Perfezionata**.
...
4. Scrive quattro file di output nella cartella `results/`, ciascuno contenente la lista degli IUN per categoria:
   ...
   * Perfezionata da meno di 120 giorni (con evento di perfezionamento e data di scadenza dei 120 giorni esatti dallo stesso)
   * Errore
