...
const endDate = '2024-12-17T23:59:46.921468518Z';

async function countItems(pkPrefix, skPrefix) {
    
    const params = {
        TableName: tableName,
        FilterExpression: 'begins_with(pk, :pkPrefix) AND contains(sk, :skPrefix) AND created >= :startDate AND created <= :endDate ',
...
async function countAppIO(enabledPrefix) {
    ...
    const params = {
        TableName: tableName,
        FilterExpression: 'begins_with(pk, :pfPrefix) AND contains(sk, :appIO) AND addresshash = :enabled AND created >= :startDate AND created <= :endDate',
        ExpressionAttributeValues: {
            ':appIO': appIO,
            ':startDate': startDate,
            ':endDate': endDate,
