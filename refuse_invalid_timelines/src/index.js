...
async function main() {
  ...
  try {
    ...
    console.log(
      "number of failing notifications: " + interestingValidations.length
    );
    ...
    for (let i = 0; i < iuns.length; i++) {
      ...
      const iun = iuns[i];
      console.log(
        `== Check notification ${i + 1} of ${iuns.length}  with iun ${iun}`
      );
      ...
      for (let f = 0; f < fileKeys.length; f++) {
        ...
        let exists = await awsClient.checkS3Exists(
          SAFE_STORAGE_BUCKET,
          fileKey
        );
